...
function AcceptedDenonciations() {
    ...
    const acceptedDenonciations = React.useMemo(() => {
        return Object.values(value)
            .filter(den => den.status)
            .filter(den => !den.isArchive)
            .filter(den => den.status === "Traité")
    }, [value])
        ...
        <motion.div
            ...
            {/* <div className="mb-5">
                <ViewTotalDenonByFilter
                    title="Total dénonciation acceptées"
                    data={acceptedDenonciations.length}
                />
            </div> */}

            <div className="mt-[1.5rem]">
                <h3 className="text-[2.2rem] font-PoppinsSemiBold">Total dénonciation acceptées</h3>
                <h3 className="text-[.85rem] mt-1">{acceptedDenonciations.length} acceptées pour le moment.</h3>
            </div>
            ...
            <div className="w-full max-lg:overflow-x-scroll max-lg:mx-auto max-lg:pr-5 max-lg:w-[90vw]">
                <table className='w-full mt-[1rem]'>
                        ...
                        <th className='w-[10px]'>
                            <input type="checkbox"
                                onChange={e => {
                                    ...
                                    let sendedValues = []
                                    e.target.checked && acceptedDenonciations?.forEach((value) => {
                    ...
                    {
                        acceptedDenonciations?.filter(pl => ((pl?.codeplainte.includes(searchInput)) ||
                            (pl?.email.includes(searchInput))))?.
                            reverse().map((value) => {
                                return (
                                    <tr
                                        // key={`${key}`}
                                        className={`hover:bg-[var(--color-bpciGray2)] transition-all cursor-pointer ${((!value.openByUsers) || (!value.openByUsers.includes(userContext.user?.email))) && 'bg-[var(--color-bpciGray2)] mb-[1rem]'}`}>
                                        <td>
                                            <input type="checkbox"
                                                onChange={(e) => {
                ...
                {
                    searchInput.length > 0 && acceptedDenonciations?.filter(pl => ((pl?.codeplainte.includes(searchInput)) || (pl?.email.includes(searchInput)))).length < 1 &&
                ...
                {
                    !Object.entries(acceptedDenonciations).length > 0 &&
                    <div className="flex items-center flex-col justify-center mx-auto mt-[1.5rem]">
                        ...
                        <p className='text-center'>Aucune dénonciation acceptée n'a été trouvé pour l'instant</p>
                    </div>
                }
            </div>
...
export default AcceptedDenonciations